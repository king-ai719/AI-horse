...
import { getPoints, consumePoint } from "@/lib/points";
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (params.get("payment") === "success") {
      import("@/lib/points").then(({ addPoints }) => {
        addPoints(6);
        setPoints(getPoints());
      });
  ...
  const handleSubmit = async () => {
    ...
    if (points <= 0) { setError("ポイントが不足しています"); return; }
    setError("");
    setLoading(true);
    const ok = consumePoint();
    if (!ok) { setError("ポイントが不足しています"); setLoading(false); return; }
    setPoints(getPoints());
    try {
      ...
      import("@/lib/points").then(({ addPoints }) => { addPoints(1); setPoints(getPoints()); });
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex gap-2 justify-center mb-8">
          {[{ label: "データ分析AI", color: "var(--race-cyan)", icon: "📊" }, { label: "展開予想AI", color: "var(--race-gold)", icon: "🎯" }, { label: "穴馬AI", color: "var(--race-red)", icon: "💥" }].map((ai) => (
        ...
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <div className="flex items-center gap-2">
            <span className="font-display font-bold text-lg" style={{ color: points > 0 ? "var(--race-cyan)" : "var(--race-red)" }}>{points}</span>
